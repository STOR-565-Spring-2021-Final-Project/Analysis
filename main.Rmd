...
```{r}
train_filtered <- train_pca[, c(1, 3, 5, 11)]
train_filtered$May <- ifelse(adopted_train$Month == "May", 1, 0)
train_filtered$Nov <- ifelse(adopted_train$Month == "Nov", 1, 0)
train_filtered$Traffic8 <- ifelse(adopted_train$TrafficType == "X8", 1, 0)
train_filtered$Traffic16 <- ifelse(adopted_train$TrafficType == "X16", 1, 0)
train_filtered$Returning <- ifelse(adopted_train$VisitorType == "Returning_Visitor", 1, 0)

test_filtered <- test_pca[, c(1, 3, 5, 11)]
test_filtered$May <- ifelse(adopted_test$Month == "May", 1, 0)
test_filtered$Nov <- ifelse(adopted_test$Month == "Nov", 1, 0)
test_filtered$Traffic8 <- ifelse(adopted_test$TrafficType == "X8", 1, 0)
test_filtered$Traffic16 <- ifelse(adopted_test$TrafficType == "X16", 1, 0)
test_filtered$Returning <- ifelse(adopted_test$VisitorType == "Returning_Visitor", 1, 0)
```
...
# Train the model
model.cvsvm <- train(Revenue~., data = train_filtered, method = "svmLinear",
...
```{r}
model.knn <- train(Revenue~., data = train_filtered, method = "knn", trControl = train.control, metric = "Kappa")
...
```
```{r}
model.lda <- lda(Revenue~PC1+PC3+PC5, data = train_filtered)

print(model.lda)

predict_lda <- predict(model.lda, test_filtered)$class
cm.lda <-
  confusionMatrix(
    factor(predict_lda),
    factor(test_filtered$Revenue),
...
```{r}
model.qda <- qda(Revenue~PC1+PC3+PC5, data = train_filtered)
print(model.qda)
