...
```{r}
for (i in c(1:10)) {
  hist(as.vector(log(dat[,i])), main=colnames(dat)[i])
}
```
...
# Train the model
model.cvsvm <- train(Revenue~PC1+PC4, data = adopted_train, method = "svmLinear",
...
```{r}
model.knn <- train(Revenue~PC1+PC4, data = adopted_train, method = "knn", trControl = train.control, metric = "Kappa")
...
```
```{r}
model.lda <- train(Revenue~PC1+PC4, data = adopted_train, method = "lda", trControl = train.control, metric = "Kappa")
...
```{r}
model.tree <- train(Revenue~PC1+PC4+Month+TrafficType+VisitorType, data = adopted_train, method = "rpart", trControl = train.control, metric = "Kappa")
...
```{r}
#SVM WIP
#model_svm = svm(Revenue~PC2+PC3, data=adopted_train, cost = 5, kernel = "linear")
#print(model_svm)
