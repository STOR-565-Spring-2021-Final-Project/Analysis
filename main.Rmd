...
# Train the model
model.cvsvm <- train(Revenue~., data = train_filtered, method = "svmRadial",
               trControl = train.control, tuneGrid = tunegrid, 
               metric = "Kappa")
# Summarize the results
print(model.cvsvm)
...
ggplot(as.data.frame(cm.tree$table),
         aes(sort(Label, decreasing = F), Prediction, fill = Freq)) + 
  geom_tile() + geom_text(aes(label = Freq)) + 
  scale_fill_gradient(low = "white") + 
  labs(x = "Label", y = "Prediction") + ggtitle("Confusion Matrix, Features")
...
tunegrid=expand.grid(C=c(seq(0.01, 2, length = 20), 10), sigma=c(seq(0.01, 2, length = 20), 10))
# Train the model
model.cvsvm <- train(Revenue~., data = train_filtered_pc, method = "svmRadial",
               trControl = train.control, tuneGrid = tunegrid, 
               metric = "Kappa")
# Summarize the results
print(model.cvsvm)
...
ggplot(as.data.frame(cm.tree$table),
  ...
  geom_tile() + geom_text(aes(label = Freq)) + 
  scale_fill_gradient(low = "white") + 
  labs(x = "Label", y = "Prediction") + ggtitle("Confusion Matrix")
```
