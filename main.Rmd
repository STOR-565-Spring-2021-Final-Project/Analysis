...
# Train the model
model.cvsvm <- train(Revenue~PC3+PC2+Month+TrafficType, data = adopted_train, method = "svmLinear",
...
```{r}
model.knn <- train(Revenue~PC2+PC3, data = adopted_train, method = "knn", trControl = train.control, metric = "Kappa")
...
```
```{r}
model.lda <- train(Revenue~PageValues+ProductRelated_Duration, data = train, method = "lda", trControl = train.control, metric = "Kappa")

predict_lda <- predict(model.lda, test)
cm.lda <-
  confusionMatrix(
    factor(predict_lda),
    factor(adopted_test$Revenue),
